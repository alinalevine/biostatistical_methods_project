...
death_data_2 = death_data_1 %>%
  dplyr::select(-c(pct_white, median_age, percent_married, pct_public_coverage,median_age_female))
...
death_data_3 = death_data_2 %>%
  dplyr::select(-c(poverty_percent,pct_emp_priv_coverage,pct_bach_deg25_over ))
...
#View(best(fit_full_3)[16,]) #use model from row 17 cp is 15.29
```

```{r}
reg = lm(formula = target_death_rate ~ incidence_rate + poverty_percent +
           avg_household_size + percent_married + pct_hs18_24 +
           pct_bach_deg18_24 + pct_hs25_over + pct_unemployed16_over + 
           pct_public_coverage_alone + pct_black + birth_rate +
           race_other + region_w + region_sw + region_me +
           region_se + sex_ratio+pct_aging, data = death_data_3)
```
