...
death_data_full = read_csv("data/Cancer_Registry.csv") %>%
  ...
  dplyr::select(-c(pct_white, pct_black, pct_other_race))
  
  
death_data = death_data_full %>%
  janitor::clean_names() %>%
  dplyr::select(-c(avg_ann_count,avg_deaths_per_year, pct_some_col18_24, binned_inc, county, incidence_rate, pct_asian))
