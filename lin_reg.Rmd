...
death_data_full = read_csv("data/Cancer_Registry.csv") %>%
  ...
  mutate(race_white_80_cutoff = ifelse(pct_white>=80, "1", "0")) %>%
  mutate(race_black_20_cutoff = ifelse(pct_black>=20, "1", "0")) %>%  #black has a lower cutoff bc the distribution is lower
  dplyr::select(-c(pct_white, pct_black))
  
  
death_data = death_data_full %>%
  janitor::clean_names() %>%
  dplyr::select(-c(avg_ann_count,avg_deaths_per_year, pct_some_col18_24, pct_other_race, binned_inc, county))
